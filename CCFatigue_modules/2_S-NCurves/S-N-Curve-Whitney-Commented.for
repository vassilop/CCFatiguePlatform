      Program Whitney
         ...
         do f=1,NOR
            ...
            call fseek(10, 0, 0) ! Scratch file > rewind to the top
            ...
            do i=1,m,1 ! For i in NOD
               ...
               if (checkmark.NE.1) then ! If new asigma
                  k=0
                  kk=1
                  strlev = strlev + 1 ! Count stress levels
                  S(strlev) = asigma(f,i)

                  if (N(f,i).LT.rrout) then ! if N < 5e7
                     k = k + 1
                     noc(strlev,k) = N(f,i)
                  end if

                  ! Copy each number of cycle for current stress level
                  do j=i+1,m,1
